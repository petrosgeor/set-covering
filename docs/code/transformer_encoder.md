...
heads and a pointer decoder. A2C training is planned but unimplemented.
...
The encoder ignores `contributions` and the external countdown. Its global input
is the mean point embedding with width `d`.

```text
Features [B,N,D+4] -> Linear -> GELU -> Linear -> [B,N,d]
                 -> pre-norm transformer layers -> LayerNorm -> H [B,N,d]
Mean(H) [B,d] -> Linear -> GELU -> Linear -> g [B,d]
```
...
Use positive dimensions, with `model_dim` divisible by `num_heads`.
